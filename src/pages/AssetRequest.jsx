...
const AssetRequest = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // UPDATED: Passing quantity to the draft endpoint
            const draftRes = await API.post(
                `/requests/draft?userId=${userId}&itemType=${itemType}&requestedDate=${requestedDate}&quantity=${quantity}`
            );
            await API.put(`/requests/${draftRes.data.id}/submit`);
            alert(`Request for ${quantity} ${itemType}(s) sent successfully!`);
            returnToDashboard();
        } catch (err) {
...
const styles = {
    ...
    qtyBtn: { width: "40px", height: "40px", borderRadius: "6px", border: "1px solid #cbd5e0", background: "#f8f9fc", cursor: "pointer", fontSize: "20px", fontWeight: "bold" },
    qtyInput: { width: "60px", textAlign: "center", padding: "10px", borderRadius: "6px", border: "1px solid #cbd5e0", fontWeight: "bold" }
};
